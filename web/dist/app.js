...
let activeJob = "";
let events = null;
const pages = ["MAP", "MODELS", "EXPLORE", "HISTORY", "USE", "SETTINGS"];
function page() {
    const raw = location.hash.replace(/^#\/?/, "").toUpperCase();
    return pages.includes(raw) ? raw : "MAP";
}
...
function statePill(state, freshness) {
...
async function renderMap() {
    ...
    try {
        const territory = await getJSON("/api/v1/territory");
      ...
      <section class="grid territory">${cells.map((c) => `
        <article class="card">
          ...
          ${(c.limitations ?? []).map((x) => `<p class="note">${escapeHTML(x)}</p>`).join("")}
        </article>`).join("")}</section>`);
    }
    catch (e) {
        fail("Territory could not be loaded", e);
    }
}
async function renderModels() {
    loading("Installed models");
    try {
        const models = await getJSON("/api/v1/models");
      ...
      <section class="grid">${models.length ? models.map(m => `<article class="card"><div class="cardhead"><h2>${escapeHTML(m.name)}</h2>${m.active_runtime ? `<span class="pill state-supported">ACTIVE</span>` : ""}</div><p>${escapeHTML(m.size)} · ${escapeHTML(m.quantization || "quantization unknown")}</p><dl><div><dt>Status</dt><dd>${m.demonstrated ? "DEMONSTRATED" : m.tested ? "TESTED" : "UNTESTED"}</dd></div><div><dt>Saved runs</dt><dd>${escapeHTML(m.saved_runs)}</dd></div><div><dt>Freshness</dt><dd>${escapeHTML(m.freshness)}</dd></div></dl>${m.tested ? `<div class="tags">${Object.entries(m.capability_states ?? {}).map(([k, v]) => `<span class="${stateClass(v)}">${escapeHTML(k)} · ${escapeHTML(v)}</span>`).join("")}</div>` : `<p class="muted">No LocalCTL evidence yet.</p>`}</article>`).join("") : `<article class="empty"><h2>No GGUF models found</h2><p>Install a GGUF under the configured LM Studio model directory, then refresh LocalCTL.</p></article>`}</section>`);
        ...
        fail("Models could not be loaded", e);
    }
}
async function renderExplore() {
    ...
    try {
        const [missions, scout, models] = await Promise.all([
            getJSON("/api/v1/missions"), getJSON("/api/v1/scout"), getJSON("/api/v1/models")
        ]);
      ...
      <h2 class="section-title">Scout</h2><section class="list">${scout.length ? scout.map(s => `<article><div><strong>${escapeHTML(s.title)}</strong><p>${escapeHTML(s.why)}</p></div><span>${s.runnable ? "RUNNABLE" : "INFORMATION"}</span></article>`).join("") : `<p class="muted">No additional gaps surfaced.</p>`}</section>`);
        ...
        fail("Explore could not be loaded", e);
    }
}
async function renderHistory() {
    loading("Evidence history");
    try {
        const [history, jobs] = await Promise.all([getJSON("/api/v1/history?limit=50"), getJSON("/api/v1/jobs?limit=20")]);
      ...
      <h2 class="section-title">Recent runs</h2><section class="list">${(history.runs ?? []).slice(0, 30).map((r) => `<article><div><strong>${escapeHTML(r.exercise_id || r.run_id)}</strong><p>${escapeHTML(r.model?.id || r.model_id || "model")} · ${escapeHTML(r.evaluation?.status || "UNKNOWN")} · ${formatDate(r.started_at || r.timestamp)}</p></div><button data-open-run="${escapeHTML(r.run_id)}">Evidence</button></article>`).join("") || `<p class="muted">No durable runs yet.</p>`}</section>`);
    }
    catch (e) {
        fail("History could not be loaded", e);
    }
}
async function renderUse() {
    loading("Bounded use recommendations");
    try {
        const items = await getJSON("/api/v1/use");
        shell(`<section class="hero"><p class="eyebrow">USE</p><h1>What can reasonably move local?</h1><p>Only current evidence that reaches LocalCTL's supported threshold appears as supported. These are advisory previews, not execution authority.</p></section><section class="grid">${items.map(x => `<article class="card"><div class="cardhead"><h2>${escapeHTML(x.title)}</h2>${statePill(x.state, "CURRENT")}</div>${x.leader ? `<p>Evidence leader: <strong>${escapeHTML(x.leader.model_name)}</strong></p>` : `<p class="muted">No supported evidence leader.</p>`}<p>${escapeHTML(x.evidence_runs ?? 0)} source runs</p>${(x.limitations ?? []).map((l) => `<p class="note">${escapeHTML(l)}</p>`).join("")}</article>`).join("")}</section>`);
    }
    catch (e) {
        fail("Use recommendations could not be loaded", e);
    }
}
async function renderSettings() {
    loading("Local boundaries");
    try {
        const s = await getJSON("/api/v1/settings");
        shell(`<section class="hero"><p class="eyebrow">SETTINGS & BOUNDARIES</p><h1>Local by construction</h1><p>The web app is a projection over LocalCTL. It does not become the source of capability truth.</p></section><section class="grid"><article class="card"><h2>Network</h2><dl><div><dt>Bind</dt><dd>${escapeHTML(s.bind_address)}</dd></div><div><dt>Runtime</dt><dd>${escapeHTML(s.runtime_url)}</dd></div></dl>${(s.security ?? []).map((x) => `<p class="ok">✓ ${escapeHTML(x)}</p>`).join("")}</article><article class="card"><h2>Evidence</h2><p class="path">${escapeHTML(s.evidence_root)}</p><p>Read index: <span class="path">${escapeHTML(s.read_index)}</span></p><p class="muted">${escapeHTML(s.read_index_authority)}</p><button data-rebuild>Rebuild read index</button></article></section>`);
    }
    catch (e) {
        fail("Settings could not be loaded", e);
    }
}
...
async function startMission(mission, model) {
    try {
        const result = await postJSON("/api/v1/jobs", { mission_id: mission, model_id: model, profile_id: "default" });
        ...
        alert(`Mission not started: ${e instanceof Error ? e.message : e}`);
    }
}
...
async function showRun(id) {
    try {
        const r = await getJSON(`/api/v1/runs/${encodeURIComponent(id)}`);
...
function wireActions() {
    document.querySelectorAll("[data-reload]").forEach(x => x.onclick = () => void boot());
    document.querySelectorAll("[data-start-mission]").forEach(x => x.onclick = () => void startMission(x.dataset.startMission || "", x.dataset.model || ""));
    document.querySelectorAll("[data-open-job]").forEach(x => x.onclick = () => void showJob(x.dataset.openJob || ""));
    document.querySelectorAll("[data-open-run]").forEach(x => x.onclick = () => void showRun(x.dataset.openRun || ""));
    document.querySelectorAll("[data-close]").forEach(x => x.onclick = () => document.querySelector("#modal")?.remove());
    document.querySelectorAll("[data-cancel]").forEach(x => x.onclick = async () => { await postJSON(`/api/v1/jobs/${encodeURIComponent(x.dataset.cancel || "")}/cancel`); document.querySelector("#modal")?.remove(); });
    document.querySelectorAll("[data-rebuild]").forEach(x => x.onclick = async () => { const r = await postJSON("/api/v1/index/rebuild"); alert(`Rebuilt read index from ${r.records} canonical observations.`); });
}
async function boot() {
    try {
        bootstrap = await getJSON("/api/v1/bootstrap");
        csrf = bootstrap.csrf_token ?? "";
        const jobs = await getJSON("/api/v1/jobs?limit=10");
        ...
        fail("Could not read the local lab", e);
    }
}
